...
\texttt{EJT178\_implant1\\recording1\_15\-03\-2022}, and neurons 124 to 223 in
the striatum.

\subsection{LDS model}

The LDS model estimates $K$ latent variables, $x_0(t),\ldots,x_{K-1}(t)$.  We
concatenated the latent variables into a latent vector $\mathbf{x}(t)$:
...
\begin{align}
    \mathbf{x}(t)=A\mathbf{x}(t-1)+\mathbf{v}(t)\quad\text{with}\ \mathbf{v}(t)\sim\mathcal{N}(0, Q)
\end{align}

The latent variables are combined linearly to approximate the firing rate of
the recorded neurons (bin size 20~ms), $\mathbf{y}(t)$, with a constant offset,
$\mathbf{a}$, and an additive noise, $\mathbf{w}(t)$.

\begin{align}
    \mathbf{y}(t)=\mathbf{a}+C\mathbf{x}(t)+\mathbf{w}(t)\quad\text{with}\ \mathbf{w}(t)\sim\mathcal{N}(0, R)
\end{align}

\begin{comment}
produced the log pre-intensity,
